...
void Setup3(LBM::Domain &fluid_dom, SCALAR::Domain &con_dom, SCALAR::Domain &temp_dom, void *UD)
{
    ...
    for(size_t ix=0; ix<fluid_dom.nx; ++ix)
    {
        ...
        for (size_t ii=0;ii<4;++ii)
        {
            ...
            vel1 = fluid_dom.Vel[ix][index-1][0];
            temp1 = temp_dom.Rho[ix][index-1][0];
            con1 = con_dom.Rho[ix][index-1][0];
            // ft[3] = 1.0 - ft[0] - ft[1] -ft[2];
            // fc[3] = 1.0 - fc[0] - fc[1] -fc[2];
            for(size_t k=0; k<temp_dom.Nneigh; ++k)
            {
                ft[k] = temp_dom.Feq(k,0.0,vel1) + ft1[k] - temp_dom.Feq(k,temp1,vel1);
                fc[k] = con_dom.Feq(k,0.0,vel1) + fc1[k] - con_dom.Feq(k,con1,vel1);  
            }
            // ft[3] = 0.5 - ft[1];
            // fc[3] = 0.5 - fc[1];
        }
        ...
        //     }
        // }
        temp_dom.F[ix][index][0][0] = 1.0 - temp_dom.F[ix][index][0][2]-temp_dom.F[ix][index][0][1]-temp_dom.F[ix][index][0][3]; 
            con_dom.F[ix][index][0][0] = 0.0 - con_dom.F[ix][index][0][2]-con_dom.F[ix][index][0][1]-con_dom.F[ix][index][0][3]; 
    }
}
...
int main () try
{
    ...
    double Ra = 1e7;//g*beta*dT*M^3/(alpha*mu)
    ...
    std::cout<<"betat = "<<temp_dom.beta<<std::endl;
    std::cout<<"betac = "<<con_dom.beta<<std::endl;
    ...
    for(size_t iy=0; iy<ny; ++iy)
    {
        ...
        if(temp_dom.IsSolid[ix][iy][0])
        {
            for(size_t k=0; k<temp_dom.Nneigh; ++k)
            {
                size_t nix = (size_t)((int)ix + (int)temp_dom.C[k](0) + (int)nx)%nx;
                size_t niy = (size_t)((int)iy + (int)temp_dom.C[k](1) + (int)ny)%ny;
                if(!temp_dom.IsSolid[nix][niy][0])
                {
                    temp_dom.IsBoundary[nix][niy][0] = true;
                }
            }
        }
        if(con_dom.IsSolid[ix][iy][0])
        {
            for(size_t k=0; k<con_dom.Nneigh; ++k)
            {
                size_t nix = (size_t)((int)ix + (int)con_dom.C[k](0) + (int)nx)%nx;
                size_t niy = (size_t)((int)iy + (int)con_dom.C[k](1) + (int)ny)%ny;
                if(!con_dom.IsSolid[nix][niy][0])
                {
                    con_dom.IsBoundary[nix][niy][0] = true;
                }
            }
        }
    }
    for(size_t ix=0; ix<nx; ++ix)
    {
        ...
        temp_dom.IsBoundary[ix][ny-1][0] = false;
        // temp_dom.IsBoundary[ix][1][0] = false;
        con_dom.IsBoundary[ix][ny-1][0] = false;
        // con_dom.IsBoundary[ix][1][0] = false;
    }

    //initial
    hid_t file_id;
    char const *Filekey = "test_porous_flow_0999.h5";
    
    file_id = H5Fopen(Filekey, H5F_ACC_RDONLY,H5P_DEFAULT);
    double *Rho = new double[nx*ny*nz];
    H5LTread_dataset_double(file_id,"/Density_0", Rho);
    double *Vel = new double [3*nx*ny*nz];
    H5LTread_dataset_double(file_id,"/Velocity_0", Vel);
    
    double *F = new double[fluid_dom.Nneigh*nx*ny*nz];
    H5LTread_dataset_double(file_id,"/F_0", F);
    double rho = 1.0;
    double temp = 1.0;
    double con = 0.0;
    ...
    for(size_t ix=0; ix<nx; ix++)
    {
        ...
        //     // con = 1.0;
        // }
        fluid_dom.Rho[ix][iy][iz] = Rho[i];
        fluid_dom.Vel[ix][iy][iz] = Vel[3*i],Vel[3*i+1],Vel[3*i+2];
        temp_dom.Rho[ix][iy][iz] = temp;
        temp_dom.Vel[ix][iy][iz] = Vel[3*i],Vel[3*i+1],Vel[3*i+2];
        con_dom.Rho[ix][iy][iz] = con;
        con_dom.Vel[ix][iy][iz] = Vel[3*i],Vel[3*i+1],Vel[3*i+2];
        fluid_dom.Check[ix][iy][iz] = 0.0;
        
        for(size_t k=0; k<fluid_dom.Nneigh; ++k)
        {
            fluid_dom.F[ix][iy][iz][k] = F[fluid_dom.Nneigh*i+k];
        }
    ...
    // Ttotal = 100;
    // tt = 1;
    Solve(fluid_dom,con_dom,temp_dom,Ttotal,tt,Setup2,Setup3,NULL,"test_cti1",true);
