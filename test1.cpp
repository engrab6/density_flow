...
void Report(LBM::Domain &fluid_dom, SCALAR::Domain &con_dom, SCALAR::Domain &temp_dom, void *UD)
{
    ...
    if(fluid_dom.Time <1e-6)
    {
        ...
        for(size_t iy = 1; iy<ny-1; ++iy)
        {
            ...
            // double sita = 1.0-temp_dom.Rho[index][iy][0];
            S += sita;
        }
        index = nx-3;
        double S1 = 0;
        ...
        {
            double sita = temp_dom.Rho[index][iy][0] - temp_dom.Rho[index+1][iy][0];
            // double sita =  temp_dom.Rho[index+1][iy][0];
            S1 += sita;
        }
        dat.oss_ss<<Util::_10_6<<fluid_dom.Time<<Util::_8s<<S<<Util::_8s<<S1<<Util::_8s<<0.5*(S+S1)<<Util::_8s<<temp_dom.Rho[50][50][0]<<std::endl;
    }
}
...
void Setup3(LBM::Domain &fluid_dom, SCALAR::Domain &con_dom, SCALAR::Domain &temp_dom, void *UD)
{
    ...
    for(size_t iz=0; iz<fluid_dom.nz; ++iz)
    {
        ...
        if(!temp_dom.IsSolid[index][iy][iz])
		{
            temp_dom.F[index][iy][iz][2] =  0.0 - temp_dom.F[index][iy][iz][0];
...
int main () try
{
    ...
    con_dom.AllocateDomainMem(nx,ny,nz,alpha,dx,dt);
    fluid_dom.Sc = 0.0;
    ...
    Solve(fluid_dom,con_dom,temp_dom,Ttotal,tt,NULL,Setup3,Report,"test1",true);
